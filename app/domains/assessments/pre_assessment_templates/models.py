...
class PreAssessmentTemplate(Base):
    __tablename__ = "pre_assessment_templates"
...
class PreAssessmentQuestion(Base):
    __tablename__ = "pre_assessment_questions"
    __table_args__ = (
        CheckConstraint(
            "question_type IN ("
            + ", ".join(f"'{t.value}'" for t in QuestionType)
            + ")",
            name="ck_pre_assessment_questions_question_type",
        ),
    )
    ...
    template_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("pre_assessment_templates.id", ondelete="CASCADE"),
        nullable=False,
    )
